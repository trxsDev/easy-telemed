...
const ACTIVE_MATCH_REQUEST_STATUSES = new Set(['waiting', 'offered', 'accepted']);
    ...
    useEffect(() => {
      const enforceSingleActiveCase = async () => {
        ...
        try {
          ...
          const hasActiveConsultation = Boolean(consultation) && !CLOSED_CONSULTATION_STATUSES.has(consultationStatus);
          const hasActiveMatchRequest = Boolean(matchRequest)
            && ACTIVE_MATCH_REQUEST_STATUSES.has(matchRequest.status)
            && (!consultation || hasActiveConsultation);

          if (belongs && (hasActiveConsultation || hasActiveMatchRequest)) {
